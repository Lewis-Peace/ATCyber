...
__uint128_t cipher(__uint128_t plain_text, __uint128_t master_key) {
    __uint128_t data = plain_text;
    for (int i = 0; i < 4; i++) {
        //printf("\nRound %i\n\n", i);
...
__uint128_t cipher_round(__uint128_t round_input, __uint128_t master_key, int round_number) {
    uint64_t round_key = key_scheduling(master_key, round_number);
    __uint128_t temp = ARK(round_input, round_key);
...
uint64_t key_scheduling(__uint128_t master_key, int round) {
    __uint128_t round_key = master_key;
    for (int i = 0; i < round; i++) {
        round_key = shift_rotate64_right(round_key ^ 0xFF3333FF, 32);
    }
    return round_key & 0x0000000000000000FFFFFFFFFFFFFFFF;
}
...
uint8_t** convert_into_matrix(uint64_t input_bits, uint8_t** input_matrix) {
    ...
    for (int i = 0; i < 4; i++) {
        for (int j = 0; j < 4; j++) {
            uint8_t cell_value = input_bits & 0xFF;
...
uint64_t decrypt(uint64_t cipher_text, __uint128_t master_key) {
...
uint64_t decrypt_round(uint64_t cipher_text, __uint128_t master_key, int round) {
