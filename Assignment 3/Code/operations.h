...
uint64_t* append_uint64(uint64_t* array, int len, uint64_t value) {
    uint64_t* new_array = malloc(sizeof(uint64_t) * (len + 1));
    for (int i = 0; i < len; i++) {
        new_array[i] = array[i];
    }
    new_array[len] = value;
...
uint8_t* append_uint8(uint8_t* array, int len, uint8_t value) {
    uint8_t* new_array = malloc(sizeof(uint8_t) * (len + 1));
    for (int i = 0; i < len; i++) {
        new_array[i] = array[i];
    }
    new_array[len] = value;
